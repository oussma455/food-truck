...
export default function StatsCards({ orders }: StatsCardsProps) {
  ...
  const stats = [
    ...
    { label: "Chiffre d'Affaires", value: `${dailyCA.toFixed(2)}€`, icon: <TrendingUp className="text-green-500" />, color: "border-green-500/20" },
  ];
    ...
    <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
      {stats.map((s, i) => (
        <motion.div 
          ...
          className={`premium-card p-6 border ${s.color} bg-white/[0.02] flex items-center gap-6`}
        >
